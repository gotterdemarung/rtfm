...
Є три способи оголосити змінну за допомогою Go:

1. Оголошення та ініціалізація дефолтом
```go
var s string // Значення - пуста строка
```

2. Оголошення та ініціалізація значенням
```go
var s string = "foo" // Значення - "foo"
```

3. Автоматиечне виведення типу 
```go
s := "foo" // Значения - "foo"
```

У 99% випадків розробники на Go оголошуючи змінну використовують третій підхід - через
...
- Референсні (як ті що в Java на хіпі)
- Примітивні (як ті що в Java на стеку)

До перших відносяться ті, що мають дефолт `nil` - массиви, слайси, мапи, канали, вказівники та функції.

До других - все інше (навіть `string`)