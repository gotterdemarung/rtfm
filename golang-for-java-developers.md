...
- [Пакети](#пакети)
  ...
  - [Власні публічні лібки](#власні-публічні-лібки)
...
❗**Починаючи з версіі 1.16 більша частина пакету `ioutil` депрекейтед та переїхала в `io` або `os`.** 
**Для читання файлу тепер краще використовувати `os.ReadFile`**
...
Як і в Java, тут код структуровано за пакетами, за кожен пакет відповідає своя папка. Пакети можуть бути
вкладеними один в одний та посилатися один на одного, але **циклічні посилання заборонені**. Тобто - якщо у вас є
пакет із назвою `foo`, що використовує щось із пакету `bar`, то сам пакет `bar` вже не зможе нічого імпортувати з
пакету `foo`. У Java це можливо, а у Go - ні. Це відкриває надзвичайні можливості кешування скомпільованих пакетів і ви
будете здивовані швидкістю компілювання - одного разу ми випадково використали дещо із пакету Nomad що призвело до того,
що весь повністю Nomad був вкомпільований в нашу реліз-тулзу, а помітили ми це лише за пів року.

У будь-якому Go-застосунку є пакети і перше, що треба зробити - оголосити головний root пакет застосунку. Це можна 
зробити у довільній папці і ця папка стане root-папкою застосунку. Для цього треба виконати команду `go mod init`,
передавши в якості аргументу ім'я головного пакету. Go тісно пов'язан із GitHub, тому у якості головних імен пакетів
для застосунків використовується ім'я їх репозитарію на GitHub, хоча це і не обов'язково. Для нашого Hello, World
зробимо просте йменування `hello`:

```shell
go mod init hello
```

Після цього у папці з'явиться файл `go.mod` у якому зберігається ім'я та й в подальшому - залежності нашого 
застосунку. Тепер ми можемо робити довільні папки і зберігати в них код (`src/main/java` - такого вже не треба). 
Наприклад якщо ми зробимо папку `util`, то код із цього пакету буде доступний у нашому застосунку за допомогою 
`import "hello/util"` (де `hello` - ім'я нашого головного пакету)

## Public/Private

Розроблюючи щось складніше за `func main()` виникає необхідність відобмежувати публічні та приватні сутності. У Go
немає ключових слів `public`, `private` та `protected`, замість цього працює правило - якщо назва будь-чого (типу, 
функції, структури, інтефейсу, тощо) починається з великої літери, то ця сутність буде доступна усюди, а якщо з 
маленької літери - то лише усередині пакету (не файлу, а саме пакету)

## Зовнішні залежності

Щоб підключити зовнішню залежність треба виконати команду `go get` та вказати в якості аргументу ім'я пакету. Наприклад 
ми могли розширити той самий Hello, World роблячи аутпут різними кольорами. Цля цього підключаємо відповідну лібку 
`go get github.com/fatih/color` та імпортуємо пакет:
...
Після підключення залежності файли `go.mod` та `go.sum` будуть модифіковані. 

## Власні публічні лібки

1. Створюєте репозитарій на GitHub, наприклад `github.com/super/lib`
2. Для свого проекту оголошуєте основний пакет із тою ж назвою `go mod init github.com/super/lib`
3. Пишете код
4. Пушите на GitHub
5. Все, більше робити нічого не треба - ваш код **вже можна качати за допомогою `go get` та імпортувати в інші проекти**


# Тестування

Функціонал для юніт тестування включено у сам Go. Для написання тесту необхідне наступне:

1. Створити файл у тій самій директорії, що й тестуюємий функціонал (той самий пакет)
2. Дати файлу назву `*_test.go`. Суффікс `_test.go` є обов'язковим
3. Створити у цьому файлі одну чи більше функцій, назва котрих починається на `Test`,
   а сама функція має лише один параметр `t *testing.T`
4. Проімпортувати пакет `testing`

У стандартному пакеті немає інструментів для ассертів, єдине, що доступно - метод `t.Errrof`,
що сигналізує про те, що тест зафейлився, відображає помилку, але продовжує виконання тест
методу.

Для запуску тесту необхідно виконати команду `go test`, якщо ж потрібно запустити тести 
рекурсивно, можна це зробити за допомогою `go test ./...`
...
Будь-яку функцію у будь-який час можна виконати "параллельно" за допомогою
ключового слова `go`, що ставиться перед викликом. Наприклад якщо замість
`fmt.Println("Hello, world!")` написати `go fmt.Println("Hello, world!")` то
відображення `"Hello, world!"` буде зроблено в окремому потоці виконання. 

Функція що виконується параллельно називається горутиною (Goroutine).

## Технічні деталі

На відмінність від Java, у Go немає абстракції `Thread` і тредів (потоків)
як таких також нема. Go використовує кооперативну багатозадачність, коли
параллельні фукнції викликаються одна за одною після завершення попередньої.
Також, будь-яка IO операція (читання файлу/мережі, сліп і т.д.) ставить 
функцію на паузу і передає керування наступній. Це дозволяє Go працювати 
одночасно без перебільшення мільйонам параллельних функцій (горутин).

У реальному житті такий підхід надзвичайно ефективний, хоча якщо сильно 
постаратися то можна зламати внутрішній скедулер горутин фукціями що обробляють
великі обсяги даних без IO.


# Посилання

- [Go by Example](https://gobyexample.com/) - усі цікавинки Go на прикладах.
